...
- **Flash feedback rendering is centralised in `_Layout`.** Controllers only set `TempData["FeedbackText"]`/`["FeedbackType"]` (the per-view inline `alert` blocks were removed in favour of this single region). The layout classifies each message: short ones render as an auto-dismissing corner toast (dismiss time scales with length, 5–12s), while long or multi-line messages (>200 chars or ≥4 lines — typically import/validation summaries built with `string.Join("<br/>", …)`) render as a dismissible inline alert at the top of the page, since long text is hard to read in a toast.
